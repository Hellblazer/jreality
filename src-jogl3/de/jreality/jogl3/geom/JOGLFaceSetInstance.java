...
public class JOGLFaceSetInstance extends JOGLLineSetInstance {
	...
	//june15 padilla: multisample texture shader
//	GLShader polygonShaderDepthMS = TransparencyHelper.depthMS;
//	GLShader polygonShaderTranspMS = TransparencyHelper.transpMS;
...
//			System.out.println("polygon shader called .render(...)");
			PolygonShader.render(fse, faceSetUniforms, faceTexture, reflMap, polygonShader, state);
		}
		
		super.render(state, width, height); // render lines and points
	...
	public void renderDepth(JOGLRenderState state, int width, int height) {
		...
		if(visible){
//			System.out.println("polygon shader called .rederDepth(...)");
			
			// june15 padilla: selecte MS or non MS shader
//			GLShader useShader = TransparencyHelper.enableMultisampleTextures ? polygonShaderDepthMS : polygonShaderDepth;
			GLShader useShader = polygonShaderDepth;
			
			PolygonShader.renderDepth(fse, useShader, state, width, height);
	...
	public void addOneLayer(JOGLRenderState state, int width, int height, float alpha) {
		...
		boolean visible = (boolean)eap.getAttribute(ShaderUtility.nameSpace(CommonAttributes.POLYGON_SHADER, CommonAttributes.FACE_DRAW), CommonAttributes.FACE_DRAW_DEFAULT);
		if(visible){
//			System.out.println("polygon shader called .addOneLayer(...)");
			
			// june15 padilla: selecte MS or non MS shader
//			GLShader useShader = TransparencyHelper.enableMultisampleTextures ? polygonShaderTranspMS : polygonShaderTransp;
			GLShader useShader = polygonShaderTransp;
			
			PolygonShader.addOneLayer(fse, faceSetUniforms, faceTexture, reflMap, useShader, state, width, height, alpha);
