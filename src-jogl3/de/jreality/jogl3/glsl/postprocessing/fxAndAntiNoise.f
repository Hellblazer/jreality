...
// ADDITIONALLY: kill of grains of noise created by the bad aliasing of far away textures
...
void main(void)
{
    ...
    if(lumaResult2 < lumaMin || lumaResult2 > lumaMax){
		...
		// TRY NOISE REDUCTION
		//if luminosity everywhere is too different, we know that our pixel is wrong
		float lumaD = 0.4; //lumaNoise maximum Difference 
		
		float lumaT = dot(luma, texture2D(image, texCoord.xy + (vec2(0.0, -1.0) * texCoordOffset)).xyz);
		float lumaR = dot(luma, texture2D(image, texCoord.xy + (vec2(1.0, 0.0) * texCoordOffset)).xyz);
		float lumaB = dot(luma, texture2D(image, texCoord.xy + (vec2(0.0, 1.0) * texCoordOffset)).xyz);
		float lumaL = dot(luma, texture2D(image, texCoord.xy + (vec2(-1.0, 0.0) * texCoordOffset)).xyz);
		...
		// avgerage color
			...
			texture2D(image, texCoord.xy + (vec2(-1.0, 1.0) * texCoordOffset)).xyz +
			texture2D(image, texCoord.xy + (vec2(1.0, 1.0) * texCoordOffset)).xyz +
			texture2D(image, texCoord.xy + (vec2(0.0, -1.0) * texCoordOffset)).xyz +
			texture2D(image, texCoord.xy + (vec2(1.0, 0.0) * texCoordOffset)).xyz +
			texture2D(image, texCoord.xy + (vec2(0.0, 1.0) * texCoordOffset)).xyz +
			texture2D(image, texCoord.xy + (vec2(-1.0, 0.0) * texCoordOffset)).xyz
			)*0.125;
		...
		if(
			abs( dot(luma , avgColor.xyz ) ) > lumaD
			//abs(lumaTL-lumaM)>lumaD &&
			//abs(lumaTR-lumaM)>lumaD &&
			//abs(lumaBL-lumaM)>lumaD &&
			//abs(lumaBR-lumaM)>lumaD 
			){
