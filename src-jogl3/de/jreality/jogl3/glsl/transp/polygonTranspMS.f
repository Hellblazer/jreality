...
//default polygon fragment shader for transparent polygons WITH MULTISAMPLING
#version 330
...
// uses sampler2DMS for multisampling
...
uniform sampler2DMS image; // is multisampled
uniform int has_Tex;
...
uniform sampler2DMS _depth; //this is multisampled
uniform int _width;
uniform int _height;
...
void main(void)
{
	...
	glFragColor = vec4(0.95,0.5,0.15,0.5);
	...
	//vec4 glTexelColors[nrOfSamples];
	...
	for(int sampleIndex = 0; sampleIndex < nrOfSamples; sampleIndex++) {
		 d = d + texelFetch(_depth, iTexCoord , sampleIndex ).x;//sampleIndex).x;
	}
	d = d / nrOfSamples;
	...
	if(abs(1-d - gl_FragCoord.z) > 0.00000001) // why abs()?
		discard; /// FOR TRANSPARENCY
	
	// average the color
	int texCoordSint = int(round(texCoord.s * _width));
	int texCoordTint = int(round(texCoord.t * _height));
	iTexCoord = ivec2(texCoordSint, texCoordTint);
	vec4 texColor = vec4(0.0);
	for(int sampleIndex = 0; sampleIndex < nrOfSamples; sampleIndex++) {
		texColor = texColor + texelFetch( image, iTexCoord , sampleIndex);
	}
	texColor = texColor/nrOfSamples;
