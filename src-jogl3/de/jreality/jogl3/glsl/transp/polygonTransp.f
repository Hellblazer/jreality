...
//default polygon fragment shader for transparent polygons
#version 330


// NOTE: very similar to usual polygon shader, but discards anything that was not the furthest away acording to the "image" texture which gives a depth map
...
void main(void)
{
	...
	// IMPORTANT: irregularities in the textures happen when the treshhold is too low
	float tresh1 = 1.0e-4; // this one has good results without destroying transparency
	float tresh2 = 1.0e-5;
	float tresh3 = 1.0e-8; // this was the old teshhold. It made distant objects look bad.
	bool inZone = false;
	// debug shit. to see where he unjustly discarded transparencies
	//if( abs(1-d - gl_FragCoord.z) < 0.00000001) {
		//glFragColor = vec4(0,1,0, transparency);
	//} else {
	
	//if(abs(1-d - gl_FragCoord.z) > 1.0e-5)//0.00000001) // why abs()?
		//discard; /// IMPORTANT FOR TRANSPARENCY
	if((1-d - gl_FragCoord.z) > tresh1) {//0.00000001) // why abs()?
		glFragColor = vec4(0,0,1, transparency);
		discard; /// IMPORTANT FOR TRANSPARENCY
	//}else if ( (1-d - gl_FragCoord.z) > tresh2 ){
	//	glFragColor = vec4(0,1,0, transparency);
	//}else if ( (1-d - gl_FragCoord.z) > tresh3 ){
		//glFragColor = vec4(1,0,0, transparency);
		//discard; /// IMPORTANT FOR TRANSPARENCY
	} else {
