...
public class WindowManager extends Plugin implements ChangeListener {
	...
	public void install(Controller c) throws Exception {
		...
		if (env != RunningEnvironment.DESKTOP){
			PortalProperties.addChangeListener(portalScaleListener = new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent arg0) {
				double yTranslation = 0;
				double ps = PortalProperties.getPortalScale();
	...
	public void uninstall(Controller c) throws Exception {
		...
		if (env != RunningEnvironment.DESKTOP && portalScaleListener != null)
			PortalProperties.removeChangeListener(portalScaleListener);
		frameRefs.clear();
		setParent(null);
	}
