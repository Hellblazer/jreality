...
/**
 * in the VR controls this adds options to change the floor
 * comment: padilla
 *
 */
public class Terrain extends Plugin implements ActionListener, ChangeListener, ColorPickerModeChangedListener {
	...
	private static final String DEFAULT_TEXTURE = DefaultJRTextures.getDefaultTexture();
	...
	private Map<String,String> textures = DefaultJRTextures.getDefaultTextures();
	...
	public Terrain() {
		...
		// the terrain node just works as a holder
		
		// define the appearance node attributes and attach it to the terrain
	...
	public Color getFaceColor() {
		return ColorConverter.toJR(faceColorChooser.getColor());
	}

	public void setFaceColor(Color c) {
		faceColorChooser.setColor(ColorConverter.toAwt(c));
	}
