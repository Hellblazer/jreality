...
/**
 * plugin.
 * loads default camera and graphical settings. creates the scenegraph.
 * is registered automatically at start up
 * @author 
 *
 */
public class Scene extends Plugin {
	
	// most plugins add themself as a changelistener to this scene list
	...
	static JrScene defaultScene() {
		...
		ShaderUtility.createRootAppearance(rootApp);
...
//		MatrixBuilder.euclidean().translate(0,0,SystemProperties.isPortal ? 0 : defaultZTranslation).assignTo(avatar);
	...
	public SceneGraphPath getDevicesPath() {
		return theScene.getPath("deviceManagerPath");
	}
	...
	public void setDeviceManagerPath(SceneGraphPath path) {
		//copy from avatar
		theScene.addPath("deviceManagerPath", path);
		fireStateChanged();
	}
	...
	/**
	 * goes through the list of registered state changes and triggers their way of updating the scene graph
	 */
	...
	public SceneGraphComponent getDevicesComponent() {
		// for deviceManagerPlugin, write this to copy the others
	...
	/**
	 * check if not null, then just does path.getLastComponent()
	 * @param path
	 * @return
	 */
...
//		ef.setDevicesPath(scene.getDevicesPath());
